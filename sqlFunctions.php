<?php
    ...
    */
    ...
    /*
        Create a teacher from a teacher object -> array [FULLNAME SPACE DELIMITED, login]
    ...
    */  
    function updateTeacher($currID, $newInfo){
        $currID = ltrim(rtrim($currID));
        $tName = ltrim(rtrim($newInfo[0]));
        $tName = explode(" ", $tName);
        $fname = $tName[0];
        $lname = $tName[1];
        $tID = ltrim(rtrim($newInfo[1]));
        if(!$currID || !$fname || !$lname || !$tID)
            return false;
        //update the teacher
        $sql = "UPDATE Instructors SET fname='$fname', lname='$lname', login='$tID' WHERE login LIKE '%$currID%'";
        if ($GLOBALS['conn']->query($sql) === TRUE) {
            echo "Teacher record created successfully <br>";
        } else {
            echo "Error: " . $sql . "<br>" . $GLOBALS['conn']->error . "<br>";
    ...
    */
    function getGraderByID($id){
        ...
        $result = mysqli_query($GLOBALS['conn'], $sql);
        if(!$result){
            echo "Grader id does not exist, did you mean to use getGraderByUsername(username)?";
            die;
        }
    ...
    */
    ...
    /*
        Create a grader from a grader object -> array [FULLNAME SPACE DELIMITED, login]
    ...
    */
    ...
    /*
        ...
        true on success
    */
    function updateGrader($currID, $newInfo){
        $currID = ltrim(rtrim($currID));
        echo "Curr id : ";
        echo $currID;
        $gName = ltrim(rtrim($newInfo[0]));
        $gName = explode(" ", $gName);
        $fname = $gName[0];
        $lname = $gName[1];
        $gID = ltrim(rtrim($newInfo[1]));
        if(!$currID || !$fname || !$lname || !$gID)
            return false;
        $sql = "SELECT id FROM Graders WHERE login LIKE '%$currID%'";
        $grader = $GLOBALS['conn']->query($sql);
        if($grader){
            $grader = $grader->fetch_assoc()['id'];
        }
        else{
            echo "Grader id does not exit";
            die;
        }

        //update the teacher
        $sql = "UPDATE Graders SET fname='$fname', lname='$lname', login='$gID' WHERE id=$grader";
        if ($GLOBALS['conn']->query($sql) === TRUE) {
            echo "Grader record created successfully <br>";
        } else {
            echo "Error: " . $sql . "<br>" . $GLOBALS['conn']->error . "<br>";
    ...
    */
    function studentsToFile($path, $section){
        $i = 0;
        $toWrite = $path;
    ...
    */
    function removeStudentS($id, $section){
        echo $id . "<br>";
        echo $section . "<br>";
    ...
    */
    function removeStudentC($id){
        ...
        $currST = $result->fetch_assoc()['id'];
        //delete the student from the section
        $sql = "DELETE FROM to_student WHERE student_id=$currST";
        echo "Current student " . $currST . "<br>";
        if(mysqli_query($GLOBALS['conn'], $sql) === TRUE)
            echo "Student removed from all sections<br>";
        else 
            echo "Failed to remove student from a section<br>";
        $sql = "DELETE FROM Students WHERE id=$currST";
        if(mysqli_query($GLOBALS['conn'], $sql) === TRUE)
            echo "Student removed entirely.<br>";
        else
            echo "Failed to remove student from Student database <br>";
    }
    /*
    ...
    */
    function getSections(){
        ...
        if($gID){
            ...
            if(!$result){
                echo "Query failed, grader might not exist or teacher or section!";
                die;
            }
    ...
    */
    function removeSection($section){
        ...
        if(($gid = $result->fetch_assoc()['grader_id'])){
            ...
            $result = mysqli_query($GLOBALS['conn'], $sql);
            removeGraderS($result->fetch_assoc()["login"], $section);
        }
        //delete the section
